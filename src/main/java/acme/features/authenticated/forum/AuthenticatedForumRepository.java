...
public interface AuthenticatedForumRepository extends AbstractRepository {
	...
	@Query("select t from Forum t where exists(select f from Forum f where f.round.entrepreneur.userAccount.id = ?1) OR exists(select a from Application a where a.investor.userAccount.id = ?1)")
